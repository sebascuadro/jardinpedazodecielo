...
$has_new_replies = $conversation->has_new_replies(get_current_user_id());
...
$thumb_icon = $reply_count == 0 ? 'fa fa-comment-o' : ($reply_count == 1 ? 'fa fa-comment' : 'fa fa-comments');
$thumb_icon = apply_filters('cuar/private-content/view/icon?addon=' . $current_addon_slug, $thumb_icon, $post);
$thumb_header = apply_filters('cuar/private-content/view/header?addon=' . $current_addon_slug, $reply_count, $post);
$thumb_sub_header = apply_filters('cuar/private-content/view/header?addon=' . $current_addon_slug, _n('REPLY', 'REPLIES', $reply_count, 'cuarme'), $post);
...
<div class="collection-item of-h mix<?php echo $extra_class; ?>">
    <div class="collection-item-wrapper panel panel-tile br-a">
        <div class="collection-list-blocks cuar-clearfix">
            <div class="collection-thumbnail collection-list-left panel-body pn <?php if (has_post_thumbnail()) { ?> thumb-active<?php } ?>">
                ...
                <table class="collection-metas table">
                    <tr data-toggle="tooltip" data-placement="top" data-original-title="<?php esc_attr_e('Started on', 'cuarme') ?>">
                        <th class="p5"><i class="fa fa-calendar"></i></th>
                        <td><?php echo get_the_date(); ?></td>
                    </tr>
                    <?php if ($has_new_replies): ?>
                        <tr data-toggle="tooltip" data-placement="top" data-original-title="<?php echo esc_attr(sprintf(__('Latest reply on %s %s', 'cuarme'), get_the_modified_time(get_option('date_format')), get_the_modified_time(get_option('time_format')))); ?>">
                            <th class="p5"><i class="fa fa-commenting-o"></i></th>
                            <td><?php _e('New replies', 'cuarme'); ?></td>
                        </tr>
                    <?php endif; ?>
                </table>
        ...
        <div class="collection-footer-metas collection-list-blocks br-t">
            <div class="collection-footer-meta-author collection-list-left" data-toggle="tooltip" data-placement="top" data-original-title="<?php esc_attr_e('Started by', 'cuarme') ?>">
                <div class="p5 table-layout"><i class="fa fa-user"></i><div><?php echo get_the_author_meta('display_name'); ?></div></div>
            </div>
            <div class="collection-footer-meta-owner collection-list-right" data-toggle="tooltip" data-placement="top" data-original-title="<?php esc_attr_e('With', 'cuarme') ?>">
